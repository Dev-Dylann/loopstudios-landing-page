...
const Footer = ({ navList }) => {
  ...
  return (
    <footer className='text-center font-alata bg-black p-6 pt-10 mt-24 lg:px-16'>
      ...
      <div className='flex flex-col items-center gap-8 max-w-7xl lg:mx-auto lg:grid lg:grid-cols-2 lg:gap-4'>
        ...
        <nav aria-label='Footer Nav' className=' text-white lg:order-1'>
            <ul className='flex flex-col gap-4 lg:flex-row'>
                {navList.map((navItem, index) => (
                  <li key={`footer${index}`} className='relative group'><a href={`#${navItem.toLowerCase()}`} >{navItem}</a><span className='absolute left-1/4 -bottom-1.5 w-1/2 h-0.5 bg-white rounded-full origin-center scale-0 group-hover:scale-100 transition-all'></span></li>
                ))}
            </ul>
        </nav>
        ...
        <section className='mt-4 flex gap-4 items-center justify-end'>
          {iconArray.map((icon, index) => (
            <a key={`icon${index}`} href="" className='relative group'><img src={icon} alt="Icon" className='w-[25px]' /><span className='absolute left-0 -bottom-2 w-full h-0.5 bg-white rounded-full origin-center scale-0 group-hover:scale-100 transition-all'></span></a>
          ))}
        </section>
